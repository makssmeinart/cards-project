...
import {
    addCardTC, deleteCardTC, editCardTC,
} from "../m2-bll/reducers/cardsReducer/cardsReducer";

// Packs Moal

export const fireDeleteModal =
    (idPack: string, namePack: string, dispatch: any) =>
        Swal.fire({
            title: 'Are you sure?',
...
export const fireEditPackModal =
    async (packId: string, packName: string, dispatch: any) => {
...
export const fireDeleteCardModal =
    (packId: string, cardId: string, dispatch: any) =>   Swal.fire({
        title: `Are you sure?`,
...
export const fireEditCardModal =
    async (cardId: string, packId: string, dispatch: any ) => {
    const {value: formValues} = await Swal.fire({
        title: 'Edit card',
        html:
            '<input placeholder="Enter new question"' +
